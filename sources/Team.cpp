...
void ariel::Team::add(ariel::Character *newChar) {
    if (newChar == nullptr) throw std::invalid_argument("got nullptr character");
    if (!newChar->isAlive()) throw std::invalid_argument("can't add a dead character to the team");
...
void ariel::Team::attack(ariel::Team *enemyTeam) {
    ...
    for (size_t i = 0; i < this->size; i++) {
        ...
        if (curr != nullptr && curr->getType() == 0) {
            if (curr->isAlive()) {
